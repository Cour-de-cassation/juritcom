...
openssl req -newkey rsa:4096 -nodes -keyout tcom-client-key.pem -out tcom-client-req.pem -subj "/C=FR/ST=Paris/L=Paris/O=MinJu/OU=Judilibre Postman/CN=CC2/emailAddress=some@mail.com"
openssl x509 -sha256 -req -in tcom-client-req.pem -days 365 -CA tcom-cert.pem -CAkey tcom-key.pem -CAcreateserial -out tcom-client-cert.pem
