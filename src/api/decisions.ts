...
import { InfrastructureError, UnexpectedError, BadFileFormat, BadFileSize, ValidationError } from '../services/error'
    ...
    try {
      ...
      const { fileName, rawfileId } = await saveDecision(
        ...
        if (error instanceof InfrastructureError) {
          req.log?.error({ ...formatLogs, message: JSON.stringify({ msg: error.message, statusCode: 503 }), stack: error.stack })
          throw error
        }
        req.log?.error({ ...formatLogs, message: JSON.stringify({ msg: error.message, statusCode: 500 }), stack: error.stack })
