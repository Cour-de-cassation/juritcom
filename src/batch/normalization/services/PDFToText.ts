...
export function HTMLToPlainText(input: string): string {
  ...
  plainText = plainText.replace(/<\/?(a|span|strong|em|i|b|u|small|sub|sup|blockquote|math)\/?>/gim, ' ');
  ...
  dom.window.document.querySelectorAll('table:has(tr:nth-child(16))').forEach((tableWithTooManyRows) => {
    if (
      /(demandeur|d[ée]fendeur|repr[ée]sent[ée]|mandataire|avocat|juge)/gim.test(tableWithTooManyRows.outerHTML) ===
      false
    ) {
      tableWithTooManyRows.outerHTML = '<p block-type="Text">[…]</p>';
    }
  });
  dom.window.document.querySelectorAll('table:has(th:nth-child(3))').forEach((tableWithTooManyColumns) => {
    if (
      /(demandeur|d[ée]fendeur|repr[ée]sent[ée]|mandataire|avocat|juge)/gim.test(tableWithTooManyColumns.outerHTML) ===
      false
    ) {
      tableWithTooManyColumns.outerHTML = '<p block-type="Text">[…]</p>';
    }
  });
  dom.window.document.querySelectorAll('table:has(td:nth-child(3))').forEach((tableWithTooManyColumns) => {
    if (
      /(demandeur|d[ée]fendeur|repr[ée]sent[ée]|mandataire|avocat|juge)/gim.test(tableWithTooManyColumns.outerHTML) ===
      false
    ) {
  ...
  const secondRegexp = /(par\sces\smotifs\W*(?:(?!par\sces\smotifs).)+l[ea]\s(?:greffi[èe]re?|pr[ée]sidente?))/i;
        ...
        if (secondRegexp.test(segments[3])) {
          ...
          if (other_segments.length > 1) {
            if (secondRegexp.test(other_segments[1]) === true) {
              plainText = plainText + other_segments[0] + ' ' + other_segments[1].split(secondRegexp)[1];
            } else {
